...
<template>
  <div
    :class="'cart_product_item row pa-30 ' + (props.good_issued != props.good_quant ? 'bg-white' : 'bg-green')"
    v-bind="$attrs"
    @click="emit('click')">
  ...
  <DialogDelete :modelValue="openDialog" title="delete_order">
    <template #content>
      <div class="text-center text-uppercase text-h3 mb-20">{{ props.good_title }}</div>
    </template>
    <template #actions>
      <RectangularButton
        :name="$t('no')"
